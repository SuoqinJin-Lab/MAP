...
def _resolve_split(paths: DatasetPaths, regime: str, split_file: str | Path | None) -> tuple[Path, dict]:
    ...
    if ("val" in payload or "test" in payload) and (
        "internal_test" not in payload or "external_test" not in payload
    ):
        raise ValueError(
            f"Legacy val/test split is not supported: {split_path}; regenerate it with create_split()"
        )
