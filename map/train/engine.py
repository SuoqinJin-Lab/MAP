...
def _resolve_split(paths: DatasetPaths, regime: str, split_file: str | Path | None) -> tuple[Path, dict]:
    manifest_path = paths.prepared / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if split_file is None:
        try:
            split_file = manifest["splits"][regime]
        except KeyError as error:
            raise ValueError(
                f"No default split registered for {regime}; pass split_file explicitly"
            ) from error
    ...
    if rule != regime:
        raise ValueError(f"Split rule {rule!r} does not match regime {regime!r}")
    return split_path.resolve(), payload
...
def train_map(
    ...
    **overrides: Any,
    ...
    for key in (
        ...
        "max_steps", "eval_every_steps", "early_stopping_patience",
    ):
        if int(params[key]) <= 0:
    ...
    for name in (train_split, val_split):
        if name not in split_payload:
            raise KeyError(f"Split set {name!r} is absent from {split_path}")
    ...
    if resume is not None:
        ...
        if allow_resource_change_on_resume:
            allowed_changes.update({"num_workers"})
        current = {
            **params,
    ...
    report = Feedback(paths.runs, f"train_{run_id}")
    module_command = [
        sys.executable,
        "-m",
        "map.train.program",
        ...
        str(mapkg_vocab),
        "--static-token-cache",
        str(paths.prepared / "map_static_tokens.pt"),
        "--num-workers",
        str(int(num_workers)),
        "--num-gene-tokens",
        str(num_gene_tokens),
    ...
    return report.finish(
        summary,
        [output / "run_config.json", output / "best.pt", output / "last.pt"],
    )
