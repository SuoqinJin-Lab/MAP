...
class CRISP(nn.Module):
    ...
    def _forward_condition(self, batch: dict, prefix: str = "") -> dict:
        cell = batch[f"{prefix}control_embeddings"].float()
        batch_size, set_size, fm_dim = cell.shape
        drug = self._indices(
            list(batch[f"{prefix}drug_smiles"]), self.smiles_to_index, cell.device
    ...
    def loss(self, output: dict, batch: dict) -> tuple[torch.Tensor, dict]:
        ...
        population_values = [list(batch["population"])]
        if "negative_prediction" in output:
            ...
            population_values.append(list(batch["negative_population"]))
        target = torch.cat(targets, dim=0)
        control = torch.cat(controls, dim=0)
        prediction = torch.cat(predictions, dim=0)
        mse = F.mse_loss(prediction, target)
        delta = target.mean(1) - control.mean(1)
        top_k = min(int(self.hparams["deg_top_k"]), self.hvg_dim)
        indices = delta.abs().topk(top_k, dim=1).indices
        mask = torch.zeros_like(delta).scatter_(1, indices, 1.0)[:, None, :]
        autofocus = ((prediction - target).square() * mask).sum(2).div(top_k).mean()
        mmd = mmd_loss(
        ...
        populations = torch.cat([
            self._indices(values, self.population_to_index, target.device)
