...
class XPert(nn.Module):
    ...
    def __init__(self, data_dir, hvg_dim: int, populations, **options) -> None:
        ...
        hvg_indices_file = assets.model_manifest.get("hvg_indices_file")
        if hvg_indices_file is None:
            if self.gene_dim != self.hvg_dim:
                raise ValueError(
                    "XPert full-gene assets require an HVG-to-gene index mapping"
                )
            hvg_indices = torch.arange(self.hvg_dim)
        else:
            hvg_indices = assets.matrix(hvg_indices_file).long().reshape(-1)
        if (
            len(hvg_indices) != self.hvg_dim
            or len(torch.unique(hvg_indices)) != self.hvg_dim
            or (hvg_indices < 0).any()
            or (hvg_indices >= self.gene_dim).any()
        ):
    ...
    def _full_expression(self, batch: dict, prefix: str) -> torch.Tensor:
        ids_key = f"{prefix}_gene_ids"
        expression_key = f"{prefix}_expressions"
        if ids_key not in batch or expression_key not in batch:
            fallback = batch[f"{prefix}_hvg_vectors"].float().mean(1)
            if self.gene_dim != self.hvg_dim:
                raise KeyError(
                    f"XPert full-gene input is missing {ids_key!r} or "
                    f"{expression_key!r}"
                )
            return fallback
        gene_ids = batch[ids_key].long()
        expression = batch[expression_key].float()
        if gene_ids.shape != expression.shape or gene_ids.ndim != 3:
            raise ValueError("XPert gene ids and expression must have shape [B, S, L]")
        valid = (gene_ids >= 0) & (gene_ids < self.gene_dim)
        batch_size, set_size, _ = gene_ids.shape
        dense = expression.new_zeros(batch_size, self.gene_dim)
        dense.scatter_add_(
            1,
            gene_ids.clamp(0, self.gene_dim - 1).reshape(batch_size, -1),
            (expression * valid.to(expression.dtype)).reshape(batch_size, -1),
        )
        return dense.div_(set_size)

    def forward(self, batch: dict) -> dict:
        control = self._full_expression(batch, "control")
        target = self._full_expression(batch, "condition")
    ...
    def loss(
        ...
        regression = (
            (treated.sqrt(), control.sqrt(), delta.sqrt())
            if initial_phase
            else (treated, control, delta)
        )
        value = (
    ...
    def predict_batch(self, batch: dict) -> torch.Tensor:
        return self(batch)["prediction"].index_select(1, self.hvg_indices)
