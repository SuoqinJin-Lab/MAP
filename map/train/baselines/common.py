...
def build_loaders(args, rank: int, world: int, dataset_class=MAPDataset):
    common = {
    ...
    validation = dataset_class(split=args.val_split, training=False, **common)
    ...
    return (
        ...
        DataLoader(validation, sampler=validation_sampler, **options),
    )
...
def distributed_mean(total: torch.Tensor) -> float:
    if dist.is_initialized():
        dist.all_reduce(total, op=dist.ReduceOp.SUM)
    return float(total[0].item() / max(total[1].item(), 1.0))
