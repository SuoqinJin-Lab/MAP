...
class CMonge(nn.Module):
    ...
    def loss(self, output: dict, batch: dict | None = None):
        ...
        fitting_terms = []
        gap_terms = []
        for source, predicted, target in zip(
            output["source_latent"],
            output["predicted_latent"],
            output["target_latent"],
        ):
            fitting_terms.append(self.fitting_loss(predicted, target))
            paired_cost = 0.5 * (source - predicted).square().sum(dim=-1).mean()
            gap_terms.append(paired_cost - self.regularized_ot(source, predicted))
        fitting = torch.stack(fitting_terms).mean()
        monge_gap = torch.stack(gap_terms).mean()
