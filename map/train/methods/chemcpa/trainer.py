...
def _checkpoint(
    model, optimizers, schedulers, args, epoch, step, early_stopping=None
) -> dict:
...
def train(args) -> None:
    ...
    if args.resume:
        ...
        validate_resume_checkpoint(checkpoint, "chemcpa")
        model.load_state_dict(checkpoint["model_state_dict"], strict=True)
    ...
    model = wrap_ddp(
        model, local=local, world=world, find_unused_parameters=True
    )

    output_dir = Path(args.output_dir)
    if rank == 0:
        write_method_config(
            output_dir, args, model="chemcpa", world=world,
            training=training,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in train_loader:
            ...
            stop_requested = early_stopping.update(
                synchronized_loss(
                    adversary_loss if adversary_step else autoencoder_loss,
                    device,
                )
            )
    ...
    if rank == 0:
        atomic_torch_save(
            _checkpoint(
                model, optimizers, schedulers, args, last_epoch, global_step,
                early_stopping,
            ),
            output_dir / "last.pt",
        )
