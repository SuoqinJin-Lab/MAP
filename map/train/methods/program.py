...
from .registry import METHOD_REGISTRY, add_model_arguments, train_model


def parse_args(argv=None):
    selector = argparse.ArgumentParser(add_help=False)
    selector.add_argument("--model", choices=tuple(name for name in METHOD_REGISTRY if name != "map"), required=True)
    selected, _ = selector.parse_known_args(argv)
    defaults = DEFAULTS[selected.model]
    parser = argparse.ArgumentParser(
        description=f"Train the {selected.model} method on MAP mmap data",
        parents=[selector],
    )
