...
to the repository's shared method runner.
"""
    ...
    def forward(
        ...
        *,
        ...
        if drug_features is not None:
            chemical = drug_features.float()
        elif drug_index is not None:
            chemical = self.drug_embeddings(drug_index)
        else:
            raise ValueError("CRISP requires drug_index or drug_features")
...
class CRISP(nn.Module):
    ...
    def _forward_condition(self, batch: dict, prefix: str = "") -> dict:
        ...
        drug_features = batch.get(f"{prefix}drug_features")
        if drug_features is not None:
            drug = None
            drug_features = drug_features.to(device=cell.device).repeat_interleave(
                set_size, dim=0
            )
        else:
    ...
    def loss(self, output: dict, batch: dict) -> tuple[torch.Tensor, dict]:
        ...
        if deg_masks:
            ...
            deg_counts = deg_mask.sum(dim=2)
            per_cell = per_cell / (deg_counts + 1e-6)
            autofocus = per_cell.sum() / (
                torch.count_nonzero(deg_counts) + 1e-6
            )
        else:
    ...
    def configuration(self) -> dict:
        return {
            ...
            "deg_mask_contract": (
                "prepared_rank_genes_groups_by_cov_t_test_abs"
                if self.hparams.get("deg_mask_mode") == "official"
                else "all_zero_map_validation"
            ),
            **self.hparams,
        }
