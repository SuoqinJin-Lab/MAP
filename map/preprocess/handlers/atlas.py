...
def _sciplex_export_root(source: Path) -> Path:
    ...
    if rds:
        raise FileNotFoundError(
            ...
            "run the SciPlex handler's export_rds() first"
        )
...
class AtlasHandler:
    """Packaged handler for native SciPlex/NIPS-style layouts."""
    def __init__(
        self,
        source: str | Path,
        projects: str | Path,
    ...
    def export_rds(
        ...
        *,
        ...
        """Export the native SciPlex RDS into the handler's matrix layout."""
        if self.kind != "sciplex":
            raise TypeError("export_rds() is only available on the SciPlex handler")
        if input_file is None:
            candidates = sorted(self.source.glob("*.RDS")) + sorted(
                self.source.glob("*.rds")
            )
            if len(candidates) != 1:
                raise FileNotFoundError(
                    f"Expected one SciPlex RDS under {self.source}; "
        ...
        if output.exists() and any(output.iterdir()):
            if not overwrite:
                raise FileExistsError(
                    f"SciPlex export directory is not empty: {output}"
                )
            shutil.rmtree(output)
        if shutil.which("Rscript") is None:
            raise RuntimeError("Rscript is required to export the SciPlex RDS")
        script = (
            Path(__file__).resolve().parents[3]
            / "scripts"
            / "export_sciplex3_rds.R"
        )
        subprocess.run(
        ...
        matrix = next(
            path
            for path in (output / "matrix.mtx.gz", output / "matrix.mtx")
            if path.is_file()
        )
        ...
        payload = {
            ...
            "cell_metadata": str(
                (output / "cell_metadata.tsv.gz").resolve()
            ),
            "gene_metadata": str(
                (output / "gene_metadata.tsv.gz").resolve()
            ),
        }
    ...
    def fetch_populations(
        ...
        *,
        ...
        contract = {
            ...
            "handler_backend": "map.preprocess.handlers.atlas_backend",
            ...
            "canonical_source": str((workspace / "materialize" / "_source").resolve()),
