...
class DataSelection:
    ...
    def prepared(self) -> Path:
        return self.cache_workspace / "materialized"
    ...
    def create_project(self, project_name: str) -> DatasetPaths:
        ...
        same_materialized_directory = destination.resolve() == self.prepared.resolve()
        if contract_file.is_file():
            ...
            if existing.get("selection_cache") != expected_cache:
                raise FileExistsError(
                    f"Materialized project belongs to another selection: {workspace}"
                )
            return experiment_paths(workspace, self.frozen_models)
        if destination.exists() and not same_materialized_directory:
            raise FileExistsError(
                f"Incomplete materialized directory has no contract: {destination}"
            )
        if self.prepared.is_dir() and not same_materialized_directory:
        ...
        Feedback(workspace, "create_project").finish(
            {
                ...
                "dataset": self.dataset,
                "populations": list(self.populations),
                "materialized_cache": str(self.prepared),
            },
