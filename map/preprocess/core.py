...
from .._common.contracts import load_contract, run_source_stage
...
def materialize_selection(
    ...
    *,
    ...
    if not (selection.prepared / "condition_filter.json").is_file():
        raise FileNotFoundError(
            "Run preprocess.<dataset>.filter_conditions(project_name) first"
        )
    if not (selection.prepared / "hvg.json").is_file():
        raise FileNotFoundError("Run preprocess.<dataset>.select_hvg(project_name) first")
    if pad_length < 2:
        raise ValueError("pad_length must contain one SPECIAL token and at least one gene token")
    paths = selection.create_project(project_name)
    return _materialize(
        paths,
        workers=workers,
        target_sum=target_sum,
        num_gene_tokens=pad_length - 1,
        overwrite=overwrite,
    )


__all__ = ["filter_conditions", "materialize_selection", "select_hvg"]
