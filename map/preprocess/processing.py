...
def materialize(
    ...
    if not filter_path.is_file():
        raise FileNotFoundError(
            "condition_filter.json is missing; run filter_conditions() before materialization"
        )
    ...
    if shapes_path.is_file() and not overwrite:
        ...
        if (
            ...
            preparation_id = preparation_identifier(
                ...
                n_top_genes=hvg_dim,
                pad_length=num_gene_tokens + 1,
                target_sum=target_sum,
                log1p=log1p,
                condition_filter_id=condition_filter["filter_id"],
            )
            preparation_manifest = paths.prepared / "preparation_config.json"
            if preparation_manifest.is_file():
                previous = json.loads(preparation_manifest.read_text(encoding="utf-8"))
                previous_filter = previous.get("condition_filter", {})
                if previous_filter.get("filter_id") != condition_filter["filter_id"]:
                    raise FileExistsError(
                        "Materialized cells were produced by another condition filter; "
                        "use a new project"
                    )
            ...
            return _result(paths, "materialize", {
                ...
                "state_pad_length": num_gene_tokens + 1,
                "gene_tokens": num_gene_tokens,
                "hvg_genes": hvg_dim,
                "target_sum": target_sum,
                "log1p": log1p,
                "dtype": dtype,
    ...
    preparation_config = {
        ...
        "pad_length": int(first_shape["token_length"]),
        "target_sum": float(target_sum),
        "log1p": bool(log1p),
        "dtype": dtype,
    ...
    preparation_id = preparation_identifier(
        ...
        pad_length=preparation_config["pad_length"],
    ...
    return _result(paths, "materialize", {
        ...
        "state_pad_length": int(next(iter(shapes.values()))["token_length"]),
        "gene_tokens": int(next(iter(shapes.values()))["token_length"]) - 1,
