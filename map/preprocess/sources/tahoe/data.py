...
def watch_data(
    ...
    *,
    ...
    for _, _, batch in reader.iter_batches(columns, batch_size):
        for line, sample, raw_drug, smiles, plate in zip(*(batch[name] for name in columns)):
            line = str(line)
            cell_lines[line] += 1
            ...
            control = drug.casefold() in CONTROL_NAMES or raw_name.casefold() in CONTROL_NAMES
            if control:
                conditions[(line, "control", "__CONTROL__", 0.0, "uM", "", str(plate))] += 1
            elif canonical and math.isfinite(dose):
                conditions[(line, "condition", drug, dose, unit, canonical, str(plate))] += 1
                drugs.add(drug)
...
def tahoe_contract(paths: TahoePaths) -> dict:
    ...
    return {
        "dataset": paths.dataset,
        "source_format": "tahoe",
        "preparation_backend": "map.preprocess.sources.tahoe.backend",
