...
def _analysis_root(paths: DatasetPaths) -> Path:
    """Keep aggregate analysis outside method run artifacts."""
    return paths.workspace / "analysis"
...
def _resolve_evaluation_file(paths: DatasetPaths, value: str | Path) -> Path:
    ...
    if path.is_file():
        return path.resolve()
    candidates = (paths.workspace / path, paths.splits / path)
...
def summarize_evaluations(
    ...
    analysis_root = _analysis_root(paths)
    report = Feedback(analysis_root, "evaluation_summary")
    rows = []
    ...
    candidates = (
        ...
        else sorted(paths.splits.glob("*/methods/*/runs/*/evaluations/*/internal_test/evaluation.json"))
        + sorted(paths.splits.glob("*/methods/*/runs/*/evaluations/*/external_test/evaluation.json"))
    )
    for path in candidates:
        ...
        evaluation_id = (
            f"{path.parent.parent.name}:{evaluation_split}"
            if evaluation_split else path.parent.name
        )
        ...
        for evaluation_unit, report_summary in reports:
            for metric, values in report_summary.items():
                row = {
    ...
    output = analysis_root / output_name
...
def analyze_predictions(
    ...
    analysis_root = _analysis_root(paths)
    report = Feedback(analysis_root, "response_analysis")
    ...
    output = analysis_root / (output_name or f"response_metrics__{analysis_id}.json")
    output.parent.mkdir(parents=True, exist_ok=True)
...
def analyze_degs(
    ...
    analysis_root = _analysis_root(paths)
    report = Feedback(analysis_root, "deg_analysis")
    rows = pq.read_table(prediction_file).to_pylist()
    output_rows = []
    for row in rows:
        predicted = np.asarray(row["predicted"], dtype=np.float64)
    ...
    out = analysis_root / "degs" / slug(analysis_id, 120)
...
def analyze_embeddings(
    ...
    analysis_root = _analysis_root(paths)
    report = Feedback(analysis_root, "embedding_analysis")
    ...
    output = analysis_root / "embeddings" / slug(analysis_id, 120)
...
def analyze_generalization(
    ...
    analysis_root = _analysis_root(paths)
    report = Feedback(analysis_root, "generalization_analysis")
    summary = []
    candidates = (
        [Path(value) for value in evaluation_files]
        if evaluation_files is not None
        else sorted(paths.splits.glob("*/methods/*/runs/*/evaluations/*/evaluation.json"))
    )
    for path in candidates:
        path = _resolve_evaluation_file(paths, path)
        if not path.is_file():
...
def build_report(
    ...
    analysis_root = _analysis_root(paths)
    root = analysis_root / "reports" / slug(analysis_id, 120)
    report = Feedback(root, "analysis_report")
    output = root / "report.html"
    summary = Path(evaluation_summary_file) if evaluation_summary_file else _analysis_root(paths) / "evaluation_summary.json"
    response = _analysis_root(paths) / f"response_metrics__{_input_identity(prediction_file)}.json" if prediction_file is not None else _analysis_root(paths) / "response_metrics.json"
