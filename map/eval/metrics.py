...
def significant_deg_mask(control, condition, fdr: float) -> np.ndarray:
    ...
    # Method 4.5.2: cell-level Wilcoxon rank-sum test followed by
    # Benjamini--Hochberg correction.  The test is deliberately performed on
    # every cell, rather than on the 24-cell pseudobulk used by the predictor.
    _, p_values = ranksums(
        control, condition, axis=0, nan_policy="omit"
    )
...
def condition_metrics(
    ...
    # Keep the full vectors for DEG ranking.  The paper ranks predicted genes
    # over all HVGs, while the reference top-k set is restricted to
    # statistically significant genes.
    full_pred_delta = predicted - control
    full_true_delta = observed - control
    pred_delta = full_pred_delta
    true_delta = full_true_delta
    ...
    if true_deg_mask is not None:
        ...
        if labels.shape != full_pred_delta.shape:
            raise ValueError("true_deg_mask must have one entry per HVG")
        auroc, auprc = _roc_metrics(labels, np.abs(full_pred_delta))
        true_top = np.flatnonzero(labels)
        true_top = true_top[
            np.argsort(-np.abs(full_true_delta[true_top]), kind="mergesort")
        ][:deg_top_k]
        predicted_top = np.argsort(-np.abs(full_pred_delta), kind="mergesort")[:deg_top_k]
        metrics.update({
            "deg_auroc": auroc,
            "deg_auprc": auprc,
            "deg_accuracy": float(
...
def perturbation_discrimination(predicted, observed, metric: str = "euclidean") -> float:
    if len(predicted) == 0:
        return float("nan")
    distances = cdist(predicted, observed, metric=metric)
    ranks = [
...
METRIC_CATALOG = {
    ...
    "hvg_pds": {
        "label": "HVG PDS (cell-line mean, Manhattan)",
        "direction": "higher",
        "definition": "Paper discrimination score (cityblock distance) averaged over cell lines",
    },
    "hvg_pds_global": {"label": "HVG PDS (global, Manhattan)", "direction": "higher"},
    "hvg_pds_euclidean": {
        "label": "HVG PDS (cell-line mean, Euclidean; diagnostic)",
        "direction": "higher",
        "definition": "Distance choice retained only for diagnostic/backward comparisons",
    },
    "hvg_pds_euclidean_global": {
        "label": "HVG PDS (global, Euclidean; diagnostic)",
        "direction": "higher",
    },
