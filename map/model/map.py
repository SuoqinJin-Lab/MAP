...
class PerturbationEncoder(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.gene_tokens_projector = ResidualProjector(2048 + self.dim_emb, self.dim_emb, 2048)
        self.transformer_backbone = map_transformer(self.num_gene_tokens, self.dim_emb, 8)
    ...
    def encode_drug(self, smiles: list[str], reference: torch.Tensor) -> torch.Tensor:
        if self._cached_smiles_to_index is not None:
            ...
            if all(index is not None for index in indices):
                cached = self._cached_drug_tokens_cpu.index_select(
    ...
    def forward(
        ...
        drug_tokens = self.encode_drug(smiles, cell_embeddings)
        ...
        doses = torch.as_tensor(doses, device=drug_tokens.device, dtype=drug_tokens.dtype).reshape(batch_size)
        if not torch.isfinite(doses).all() or (doses < 0).any():
            raise ValueError("Drug doses must be finite and non-negative")
        drug_tokens = drug_tokens * torch.log10(doses + 1.0).unsqueeze(-1)
        ...
        encoded = self.transformer_backbone(inputs_embeds=combined).last_hidden_state[:, 1]
        predicted_embeddings = self.project_out(encoded)
