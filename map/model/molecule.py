...
class MoleculeTransformer(nn.Module):
    ...
    def encode(self, ids: torch.Tensor, padding: torch.Tensor) -> torch.Tensor:
        # MolBART scales token embeddings by sqrt(d_model) before adding
        # positional embeddings (the Fairseq Transformer convention).
        values = self.emb(ids) * math.sqrt(self.d_model)
        values = values + self.pos_emb[:ids.shape[0], None, :]
