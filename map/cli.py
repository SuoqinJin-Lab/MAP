...
def _native_project_commands(dataset_parser, fetch_name: str, population_flag: str):
    ...
    materialize.add_argument("--num-gene-tokens", type=int, default=2_048)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    split.add_argument("--test-size", type=_size, required=True)
    split.add_argument("--val-size", type=_size, required=True)
    ...
    baselines.add_argument("--xpert-hidden-size", type=int, default=128)
    baselines.add_argument("--xpert-layers", type=int, default=3)
    baselines.add_argument("--xpert-epochs", type=int, default=100)
    baselines.add_argument("--xpert-seed", type=int, default=42)
    ...
    training.add_argument("--eval-every-steps", type=int)
...
def main(argv=None) -> None:
    ...
    if args.stage == "preparation":
        if args.command == "create-workflow":
            ...
            preparation.create_split(
                workflow, rule=args.rule, test_size=args.test_size,
    ...
    if args.stage == "train":
        ...
        for name in (
            ...
            "eval_every_steps", "early_stopping_patience", "seed",
        ):
