...
def _embed_population(
    ...
    *,
    ...
    if output is None:
        output_path.touch()
    cursor = 0
    total_batches = max(1, (partition_cells + batch_size - 1) // batch_size)
    ...
    with torch.inference_mode():
        for batch_index, (gene_ids, expression) in enumerate(loader):
            ...
            if (
                ...
                print(
                    f"[state] {population} partition "
                    f"{partition_index + 1}/{num_partitions}: "
                    f"{cursor:,}/{partition_cells:,} rows "
                    f"({cursor / max(1, partition_cells):.1%})",
                    flush=True,
                )
...
def _embed_static_kind(
    ...
    if kind == "genes":
        ...
        parts = []
        total_batches = max(1, (len(indices) + batch_size - 1) // batch_size)
            ...
            for start in range(0, len(indices), batch_size):
                ...
                if batch_index == 1 or batch_index % progress_every == 0 or batch_index == total_batches:
                    report.emit(
                        ...
                        kind=kind,
                        partition=f"{partition_index + 1}/{num_partitions}",
                        batches=f"{batch_index}/{total_batches}",
                    )
        tokens = (
        ...
        total_batches = max(1, (len(order) + batch_size - 1) // batch_size)
        progress_every = max(1, total_batches // 20)
        with torch.inference_mode(), _precision(device):
...
def merge_static_tokens(
    ...
    output = paths.prepared / "knowledge_tokens.pt"
