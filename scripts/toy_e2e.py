...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--hvg-dim", type=int, default=32)
    parser.add_argument("--num-gene-tokens", type=int, default=32)
...
def make_toy_xpert_inputs(workflow, *, seed: int) -> dict[str, Any]:
    ...
    root = workflow.prepared / "baselines"
    root.mkdir(parents=True, exist_ok=True)
    manifest_file = root / "manifest.json"
    manifest = (
        json.loads(manifest_file.read_text(encoding="utf-8"))
        if manifest_file.is_file()
        else {"format_version": 3, "models": {}}
    )
    ...
    directory = root / "xpert"
    directory.mkdir(parents=True, exist_ok=True)
    rng = np.random.default_rng(seed)
    hidden = min(16, max(4, hvg_dim))
    np.save(directory / "ppi_gene_vectors.float32.npy", rng.normal(size=(hvg_dim, hidden)).astype(np.float32))
    np.save(directory / "drug_hg_embeddings.float32.npy", rng.normal(size=(len(smiles), hidden)).astype(np.float32))
    unimol = np.zeros((len(smiles), 8, 514), dtype=np.float16)
    unimol[:, :4, 0] = 1
    unimol[:, :4, 1] = 20
    unimol[:, :4, 2:] = rng.normal(size=(len(smiles), 4, 512)).astype(np.float16)
    np.save(directory / "drug_unimol.float16.npy", unimol)
    payload = {
        "model": "xpert",
        "directory": "xpert",
    ...
    manifest.setdefault("models", {})["xpert"] = payload
    manifest["drug_vocabulary"] = smiles
...
def main() -> None:
    ...
    if args.dataset == "tahoe":
        ...
        recorder.run(
            "statistics",
            lambda: preprocess.tahoe.statistics(storage=args.root, batch_size=32),
        )
        recorder.run(
            "fetch_cell_line",
            lambda: preprocess.tahoe.fetch_cell_line(
                populations, project_name="toy-e2e",
                storage=args.root, batch_size=32,
            ),
        )
        dataset_preprocess = preprocess.tahoe
        ...
        recorder.run(
            "statistics",
            lambda: preprocess.sciplex.statistics(storage=args.root),
        )
        recorder.run(
            "fetch_cell_line",
            lambda: preprocess.sciplex.fetch_cell_line(
                populations, project_name="toy-e2e", storage=args.root
            ),
        )
        dataset_preprocess = preprocess.sciplex
    else:
        ...
        recorder.run(
            "statistics",
            lambda: preprocess.nips.statistics(storage=args.root),
        )
        recorder.run(
            "fetch_cell_type",
            lambda: preprocess.nips.fetch_cell_type(
                populations, project_name="toy-e2e", storage=args.root
            ),
        )
        dataset_preprocess = preprocess.nips
    recorder.run(
        "select_hvg",
        lambda: dataset_preprocess.select_hvg(
            "toy-e2e", storage=args.root,
            n_top_genes=args.hvg_dim, workers=1,
        ),
    )
    recorder.run(
        "materialize",
        lambda: dataset_preprocess.materialize(
            "toy-e2e", storage=args.root,
    ...
    workflow = recorder.run(
        "create_workflow",
        lambda: preparation.create_workflow("toy-e2e", storage=args.root),
    )
    recorder.run(
    ...
    recorder.run(
        ...
        lambda: preparation.precache_gene_tokens(workflow, batch_size=512),
    )
    recorder.run(
        "precache_drug_tokens",
        lambda: preparation.precache_drug_tokens(workflow, batch_size=4),
    )
    recorder.run(
        "merge_knowledge_tokens",
        lambda: preparation.merge_knowledge_tokens(workflow),
    )
    recorder.run(
        "precache_state_embeddings",
        lambda: preparation.precache_state_embeddings(
            workflow, batch_size=8, workers=0
        ),
    )
    recorder.run(
        "merge_state_embeddings",
        lambda: preparation.merge_state_embeddings(workflow),
    )
    recorder.run(
    ...
    if baseline_models:
        recorder.run(
            ...
            lambda: preparation.prepare_baseline_inputs(
                workflow,
        ...
        if args.xpert_assets == "real":
            recorder.run(
                ...
                lambda: preparation.prepare_baseline_inputs(
                    ...
                    models=("xpert",),
                    overwrite=True,
                    xpert_reference_dir=args.frozen_models.parent
                    / "reference_data"
                    / "xpert",
                    xpert_unimol_dir=args.frozen_models / "unimol",
                    xpert_hidden_size=min(16, args.hvg_dim),
                    xpert_layers=3,
                    xpert_epochs=1,
                    xpert_seed=args.seed,
                    xpert_device="cpu",
                ),
            )
        else:
    ...
    for model in args.models:
        ...
        evaluation_files = require_files([
            ...
            Path(evaluation.summary["prediction_files"][0]),
            Path(evaluation.summary["evaluation_dir"]) / "runs.csv",
        ])
        analysis = recorder.run(
            f"analyze_{model}",
            lambda: eval.analyze(
                workflow,
                prediction_file=evaluation.summary["prediction_files"][0],
                evaluation_files=[evaluation.summary["evaluation_file"]],
                output_name=f"toy-{model}",
            ),
        )
        report_file = workflow.evaluations / "reports" / f"toy-{model}" / "report.html"
        analysis_files = require_files([report_file])
        evaluation_payload = json.loads(
            Path(evaluation.summary["evaluation_file"]).read_text(encoding="utf-8")
        )
