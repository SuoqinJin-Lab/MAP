...
def make_toy_xpert_inputs(workflow, *, seed: int) -> dict[str, Any]:
    ...
    gene_count = int(hvg_indices.max()) + 1
    gene_symbols = all_gene_symbols[:gene_count]
    if (
        len(hvg_indices) != hvg_dim
        or len(np.unique(hvg_indices)) != hvg_dim
        or np.any(hvg_indices < 0)
        or np.any(hvg_indices >= gene_count)
    ):
        raise ValueError("Toy XPert HVG-to-full-gene mapping is invalid")
    directory = root / "xpert"
    directory.mkdir(parents=True, exist_ok=True)
    rng = np.random.default_rng(seed)
    hidden = min(16, max(4, gene_count))
    np.save(
        directory / "ppi_gene_vectors.float32.npy",
        rng.normal(size=(gene_count, hidden)).astype(np.float32),
    )
    np.save(directory / "hvg_indices.int32.npy", hvg_indices)
...
def main() -> None:
    ...
    recorder.run(
        "filter_conditions",
        lambda: dataset_preprocess.filter_conditions(
            "toy-e2e",
            storage=args.root,
            min_cells=1,
