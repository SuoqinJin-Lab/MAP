...
def test_projects_are_independent_and_created_by_materialization(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("map.preprocess.core._materialize", lambda *args, **kwargs: "ok")
...
def test_identifiers_expose_high_value_parameters():
    split_id = split_identifier("unseen_combination", 0.05, 0.05, 42)
    assert split_id == (
        "unseen_combination__test-p0p05__val-p0p05__seed-42__testdisjoint"
    )
    run_id = training_identifier(
        "unprofiled_drug",
        "unprofiled_drug__test-n16__val-n16__seed-42",
        set_size=24,
        batch_size=1,
    ...
    assert "ckpt-example-best" in evaluation_id
...
def test_parameterized_split_and_train_dry_run(tmp_path):
    ...
    assert split_path.name == "unprofiled_drug__test-n1__val-n1__seed-123.json"
    payload = json.loads(split_path.read_text(encoding="utf-8"))
    assert payload["split_id"] == split_path.stem
    assert payload["counts"] == {"train": 4, "val": 1, "test": 1}
    manifest = json.loads(
...
def test_train_registry_defaults_to_map_and_baselines_dry_run(tmp_path):
    ...
    population = "A549"
    (paths.prepared / "materialized_shapes.json").write_text(
        json.dumps({population: {"n_cells": 32, "token_length": 2049, "hvg_dim": 2000}}),
        encoding="utf-8",
    )
    ...
    (paths.prepared / "manifest.json").write_text(json.dumps({
        ...
        "materialized_shapes": {population: {"n_cells": 32, "token_length": 2049, "hvg_dim": 2000}},
    }), encoding="utf-8")
...
def test_baseline_inputs_are_prepared_once_before_training(tmp_path):
    ...
    for model in selected_models:
        ...
        instance = build_model(model, paths.prepared, 3, ["A549"])
        loss = instance.loss(instance(batch), batch)
        ...
        restored = load_model(
            model, checkpoint, paths.prepared, 3, ["A549"], torch.device("cpu")
        )
        restored.eval()
        assert restored.predict_batch(batch).shape == (1, 3)
...
def test_eval_registry_supports_map_and_baselines_dry_run(tmp_path):
    ...
    for model in ("map", *train.MODEL_REGISTRY):
        ...
        result = eval.run(
            paths, run_name=run_name, seeds=(42,), set_size=1,
